...
public class LedgerAccountController {
    ...
    public ResponseEntity<?> lookupAccount(@RequestHeader(value = "uuid", required = true,defaultValue = "f4b14e1c-0d80-11e7-93ae-92361f002671") String uuid, @RequestHeader(required = true) String userAccountId) {
        ...
        return accountDelegate.fetchAccounts(userAccountId,responseHeaders);
    }
    ...
    public ResponseEntity<?> insertOrUpdateAccounts(@ApiParam(name = "uuid", value = "A 128 bit universally unique identifier (UUID) that you generate for every request and is used for tracking.", defaultValue = "f4b14e1c-0d80-11e7-93ae-92361f002671") @RequestHeader(value = "uuid") String uuid,
                                                    @RequestBody @Valid AccountRequest request,@RequestHeader(required = true) String userAccountId
        ...
        if (accounts != null) {
            ...
            collectAccts = accounts.stream().filter(account -> userAccountId.equals(account.getUserAccountId())).collect(Collectors.toList());
